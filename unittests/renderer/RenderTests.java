 ...
 * <p>
 * These tests demonstrate the full rendering pipeline: scene construction →
 * camera setup → ray tracing → image generation.
 * <p>
 * The first test produces a simple scene intended as a reference image for
 * validating Camera and Renderer implementations.
 */
...
class RenderTests {
	...
	Camera renderSceneXML(Camera.Builder builder, String xmlName) {
		...
		// Call the parser to read the file and update the scene
		// Path expects the file to be inside an "xml" folder at the root of the project
		SceneXmlParser.parse(scene, "xml/" + xmlName + ".xml");
